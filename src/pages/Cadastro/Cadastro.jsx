...
const Cadastro = () => {
  //para criar o usuário já com esses atributo. Posteriormente apenas são editados
  const [cep, estado, bairro, cidade, nCasa, agencia, nConta, pix] = "";

  const [cpf, setCpf] = useState("");
  const onChangeCPF = (ev) => {
    const originalCpf = unMask(ev.target.value);
    const maskedCpf = mask(originalCpf, [
      "999.999.999-99",
      "99.999.999/9999-9",
    ]);
    setCpf(maskedCpf);
  ...
  const onChangeTel = (ev) => {
    ...
    const maskedTel = mask(originalTel, [
      "(99) 9 9999-9999 ",
      "(99) 9999-9999",
    ]);
  ...
  const handleSubmit = (e) => {
    ...
    fetch("http://localhost:3001/clientes", {
      ...
      .then((res) => {
        usenavigate("/login");
        //aqui a gente vai redirecionar pra tela de login
      })
      .catch((error) => {
